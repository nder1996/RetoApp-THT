...
export class TodoComponent implements OnInit {
  ...
  updateItemDetails(): void {
    ...
    this.itemsClient.updateTodoItemDetail2(this.selectedItem.id, item).subscribe(
      () => {
  ...
  updateItem(item: TodoItemDto, pressedEnter: boolean = false): void {
    ...
    if (item.id === 0) {
      this.itemsClient
        .createTodoItemV2({ title: item.title, listId: this.selectedList.id } as CreateTodoItemCommand)
        .subscribe(
          result => {
            item.id = result;
          },
      ...
      this.itemsClient.updateTodoItem2(item.id, item as UpdateTodoItemCommand).subscribe(
  ...
  deleteItem(item: TodoItemDto) {
    ...
    if (item.id === 0) {
      ...
      this.itemsClient.deleteTodoItem2(item.id).subscribe(
        () =>
          (this.selectedList.items = this.selectedList.items.filter(
            t => t.id !== item.id
          )),
