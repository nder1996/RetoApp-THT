...
async cargarTareas() {
  try {
    this.tasks = await this.taskRepository.getAllTask();
  } catch (error) {
...
async agregarTarea(nuevaTarea: TaskRequest) {
  try {
    await this.taskRepo.insert(nuevaTarea);
...
async eliminarTarea(id: number) {
  try {
    await this.taskRepo.inactivateById(id);
...
async actualizarTarea(id: number, tareaActualizada: TaskRequest) {
  try {
    await this.taskRepo.update(id, tareaActualizada);
