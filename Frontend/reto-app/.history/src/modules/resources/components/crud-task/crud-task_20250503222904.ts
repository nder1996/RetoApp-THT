...
export class CrudTask {
  ...
  public selectDetele: TaskResponse;
  public selectEdit: ITask;
  public listStatus: ITaskStatus[] = [];
  public EditModal: boolean = false;

  public modalDataForm:TaskResponse
  ...
  validateForm() {
    ...
    if (!this.taskForm.title || this.taskForm.title.trim() === '') {
      ...
      isValid = false;
    } else if (!/^[a-zA-Z0-9\s]{3,50}$/.test(this.taskForm.title)) {
    ...
    if (!this.taskForm.description || this.taskForm.description.trim() === '') {
      ...
      isValid = false;
    } else if (this.taskForm.description.length < 10) {
    ...
    if (!this.taskForm.dueDate) {
      ...
      isValid = false;
    } else if (!/^\d{4}-\d{2}-\d{2}$/.test(this.taskForm.dueDate)) {
