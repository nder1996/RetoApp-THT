...
export class CrudTask {
  ...
  editTask(task) {
    ...
    // Crear una nueva instancia de TaskRequest y mapear correctamente las propiedades
    ...
    // Asegúrate de que dueDate sea un objeto Date válido
    if (task.dueDate) {
      // Convertir a objeto Date si es string
      this.modalDataForm.DueDate = task.dueDate instanceof Date ? 
        new Date(task.dueDate) : 
        new Date(task.dueDate);
    
      // Verificar si la fecha es válida
      if (isNaN(this.modalDataForm.DueDate.getTime())) {
        console.error('Fecha inválida:', task.dueDate);
        this.modalDataForm.DueDate = new Date(); // Usar fecha actual como fallback
      }
    
      console.log('Fecha formateada para input:', this.formatDateForInput(this.modalDataForm.DueDate));
    } else {
      this.modalDataForm.DueDate = new Date(); // Usar fecha actual si no hay fecha
    }
    
    // Verificar si status existe y tiene la propiedad name antes de intentar acceder a ella
    if (task.status && task.status.name) {
      this.modalDataForm.Status = task.status.name.toUpperCase();
    } else if (task.status) {
      // Si status existe pero no tiene name, intentar usar directamente el status
      this.modalDataForm.Status = typeof task.status === 'string' ? task.status.toUpperCase() : '';
    } else {
      // Si no hay status, usar un valor por defecto
      this.modalDataForm.Status = '';
    }
    
    this.modalDataForm.State = "A"; // Asumiendo que "A" significa activo
    
    this.EditModal = true;
    console.log('Editando tarea modalDataForm:', this.modalDataForm);
    console.log('Fecha de vencimiento:', this.modalDataForm.DueDate);
    console.log('Fecha formateada:', this.formatDateForInput(this.modalDataForm.DueDate));
  }
  ...
  createTask() {
    // Asegurarnos que la fecha se envíe en formato ISO string
  ...
  async cargarTareas() {
    try {
      ...
      if (response) {
        ...
        console.log("lista de cargar Tareas : "+JSON.stringify)
      }
    } catch (error) {
